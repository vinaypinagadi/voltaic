...
from app.core.memory_db import profiles, tickets, fan_chats, add_alert
...
async def chat_stream(request: ChatRequest, user: dict = Depends(get_current_user)):
    ...
    if emergency:
        ...
        ticket_data = tickets.get(user["id"])
        
        try:
            ticket_res = supabase.table("tickets").select("*").eq("user_id", user["id"]).execute()
            if ticket_res.data:
                ticket_data = ticket_res.data[0]
        except Exception as e:
            print(f"Supabase offline. Locating via memory: {e}")
        ...
        try:
            ...
            print(f"Failed to insert emergency alert into Supabase: {e}")

        try:
            supabase.table("fan_chats").insert({
                "user_id": user["id"],
                "message": message,
                "response": emergency["response"]
            }).execute()
        except Exception as e:
            print(f"Failed to save emergency chat record: {e}")
    ...
    async def response_stream():
        full_chunks = []
        for chunk in agent.generate_chat_stream(message, user["id"], history):
        ...
        try:
            ...
            print(f"Failed to write fan chat transaction: {e}")
